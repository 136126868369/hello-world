...
void DisableDog(void)
{
	...
	//    EDIS;
		//28335 BUG,上电复位访问XINFT有概率死机,上电后需要强制WD复位一次即可
		if(!(SysCtrlRegs.WDCR & 0x0080))
		{
			EALLOW;
			SysCtrlRegs.WDCR = 0x00A8;	//Force a reset,make sure Wdog is enable
			SysCtrlRegs.WDCR = 0x0000;	//Write invalid data force reset
			EDIS;
		}
		EALLOW;
		SysCtrlRegs.WDCR= 0x0068;
